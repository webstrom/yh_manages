...
/***/ "./public/index/js/manages_system_role.js":
/*!************************************************!*\
  !*** ./public/index/js/manages_system_role.js ***!
  \************************************************/
...
new _vue2.default({
	...
	data: {
		authorization: {
			system: [{ name: '添加' }, { name: '修改' }, { name: '删除' }, { name: '上架' }, { name: '下架' }, { name: '充值' }, { name: '暂停注册' }],
			account: [{ name: '添加' }, { name: '修改' }, { name: '删除' }, { name: '启用' }, { name: '禁止' }, { name: '新增渠道用户' }, { name: '隐藏手机号' }, { name: '显示手机号' }, { name: '渠道授权查看' }],
			role: [{ name: '添加' }, { name: '修改' }, { name: '删除' }, { name: '授权' }, { name: '新增渠道角色' }],
			meun: [{ name: '添加' }, { name: '修改' }, { name: '删除' }, { name: '按钮管理' }],
			log: [{ name: '下载后台日志' }, { name: '手动备份' }, { name: '删除' }, { name: '下载操作日志' }, { name: '运维日志下载' }, { name: '问题查询' }],
			service: [{ name: '确定' }, { name: '服务设置2' }],
			salesman: [{ name: '添加' }, { name: '修改' }, { name: '删除' }, { name: '启用' }, { name: '禁用' }]
		},
		checkboxData: [],
		checkboxList: [],
		checked: false,
	...
	methods: {
		roleList: function roleList() {
			...
			$.ajax({
				url: _common2.default.manage_url + 'role/roleList.htm',
				data: data,
				type: 'post',
				success: function success(res) {
					...
					if (res.rows instanceof Array) {
						_vue2.default.set(that, 'checkboxData', res.rows);
						_vue2.default.set(that, 'total', res.total);
						that.managesPage();
		...
		addRole: function addRole(e) {
			var that = this;
			var html = '添加角色信息';
			if ($(e.target).html() === '修改') {
				html = '修改角色信息';
				if (that.checkboxList.length !== 1) {
					_common2.default.layerMsg('请选择单个角色进行操作');
					return false;
				}
			}
			...
			window.parent.layui.use('layer', function () {
				...
				layer.open({
					...
					yes: function yes(number, determine) {
						...
						$.ajax({
							url: _common2.default.manage_url + 'role/reserveRole.htm',
							data: data,
							type: 'post',
							success: function success(res) {
								_common2.default.NoLogin(res);
								res = JSON.parse(res);
								if (res.errorMsg === undefined) {
									_common2.default.layerMsg($(e.target).html() + '成功！', 1);
									that.roleList();
									layer.close(number);
								} else {
			...
			window.parent.layui.use('layer', function () {
				...
				layer.open({
					...
					yes: function yes(number) {
						...
						console.log(data);
						$.ajax({
							url: _common2.default.manage_url + 'role/deleteRole.htm',
							type: 'post',
							data: data,
							success: function success(res) {
								var jsonPack = JSON.stringify(res.replace(/[ ]/g, "").trim());
...
/***/ 3:
/*!****************************************************************************************************************************************************************************!*\
  !*** multi ./node_modules/_webpack-dev-server@3.1.14@webpack-dev-server/client?http://127.0.0.1:8088 (webpack)/hot/dev-server.js ./public/index/js/manages_system_role.js ***!
  \****************************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
module.exports = __webpack_require__(/*! E:\web\yh_manages\public\index\js\manages_system_role.js */"./public/index/js/manages_system_role.js");


/***/ })

/******/ });
//# sourceMappingURL=manages_system_role.js.map